...
//                tempP1 = new Pos((double) ev.getX(), (double) ev.getY());
                break;
    ...
    Pos p0 = new Pos(0, 0);//最后一次坐标点
    Pos p = new Pos(0, 0);//最后一次坐标点
    float dy = 0;//下移总量
    float dx = 0;//右移总量
...
//
    ...
    private void moveToDest(int nextId) {
        ...
        mAniHelper.startAni(new Pos(getScrollX(), 0), new Pos(distance, 0));
